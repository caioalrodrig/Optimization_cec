...
if __name__ == '__main__':
    ...
    print(obj.LEN_POP)
    pso = PSO_run(D = D, iterations = obj.SOL_PER_EXEC, LEN_PART = obj.LEN_POP, u_bound = b , l_bound= -b )
    pso_objective = pso.objective_function
    best_fitnesses = []
        

    for i in range(N_independ_exec):
        optimizer = ps.single.GlobalBestPSO(n_particles= 45, dimensions= D, options={'c1': 0.5, 'c2': 0.5, 'w': 0.9}, bounds= (-b*np.ones(D),b*np.ones(D)))
